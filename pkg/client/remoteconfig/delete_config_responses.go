...
func (o *DeleteConfigReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	...
	case 500:
		...
		return nil, result
	default:
		return nil, runtime.NewAPIError("[DELETE /api/remote-config/logs-pipeline] deleteConfig", response, response.Code())
	}
}
...
func (o *DeleteConfigOK) Error() string {
	...
	return fmt.Sprintf("[DELETE /api/remote-config/logs-pipeline][%d] deleteConfigOK %s", 200, payload)
}
...
func (o *DeleteConfigInternalServerError) Error() string {
	payload, _ := json.Marshal(o.Payload)
	return fmt.Sprintf("[DELETE /api/remote-config/logs-pipeline][%d] deleteConfigInternalServerError %s", 500, payload)
}
